...
from src.kessler_game.kessler_game import KesslerGame, Scenario
...
class T1Controller(KesslerController):
    @property
    def name(self) -> str:
        return "Team 1 Controller"

    def actions(self, ownship: Ship, input_data: Dict[str, Tuple]) -> None:
        # timeout(input_data)
        ...
        if ownship.team == 1:
            ownship.turn_rate = random.uniform(ownship.turn_rate_range[0]/1.0, ownship.turn_rate_range[1])
...
class T2Controller(KesslerController):
    @property
    def name(self) -> str:
