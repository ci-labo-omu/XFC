...
class NewController(KesslerController):
    ...
    def actions(self, ownship: Ship, input_data: Dict[str, Tuple]) -> Tuple[float, float, bool]:
        ...
        for i in range(len(search_list)):
            ...
            if search_list[i]['size'] == 4:
                self.Con4_sim.input['A4'] = search_dist[i]
                self.Con4_sim.input['B4'] = abs(angdiff)
                self.Con4_sim.compute()
                Risk[i] = self.Con4_sim.output['Con4']
            elif search_list[i]['size'] == 3:
                self.Con3_sim.input['A3'] = search_dist[i]
                self.Con3_sim.input['B3'] = abs(angdiff)
                self.Con3_sim.compute()
                Risk[i] = self.Con3_sim.output['Con3']
            elif ast_list[i]['size'] == 2:
                self.Con2_sim.input['A2'] = search_dist[i]
                self.Con2_sim.input['B2'] = abs(angdiff)
                self.Con2_sim.compute()
                Risk[i] = self.Con2_sim.output['Con2']
            elif ast_list[i]['size'] == 1:
