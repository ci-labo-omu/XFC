...
class Problem1(Problem):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        f1_rank = rankdata(self.values_f1, method="ordinal")[-100:]
        f2_rank = rankdata(self.values_f2, method="ordinal")[-100:]
...
def main():
    ...
    for i in range(100):
        ...
        nds = non_dominated(F)
        print(len(nds))
        title = (f"Rank All Archive method gen={NGEN} (seed={i})")

        nondom_f1 = [item[0] for item in nds]  # 左側の要素のリスト
        nondom_f2 = [item[1] for item in nds]  # 右側の要素のリスト
        plot_free(F[:, 0], F[:, 1], title)
        plot_free(nondom_f1, nondom_f2, title)
