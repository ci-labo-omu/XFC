...
if __name__ == "__main__":
    ...
    ave_list = []
    accu = [run3worst(genes[j]) for j in range(len(genes))]

    for i in range(2500):

        gene_a, gene_b = tournament(genes, accu)
        child = blend(gene_a, gene_b, 0.5)
        accu_min = np.argmin(accu)
        accu_child = run3worst(child)
        if accu_child > np.min(accu):
            genes[accu_min] = child
            accu[accu_min] = accu_child
        ...
        with open("run3worst_final.csv", "a", newline='') as f:
            writer = csv.writer(f)
